...
class Agent():
    ...
    def recall(self):
        ...
        # Normalize states
        state_batch = (torch.tensor(np.array(batch.state), dtype=torch.float).unsqueeze(1) / 255.0).to(device)
        
        # Normalize next_states
        next_state_batch = (torch.tensor(np.array(batch.next_state), dtype=torch.float).unsqueeze(1) / 255.0).to(device)
...
def train(env, episodes):
    ...
    max_ret = 0
    obs = env.reset()
    obs_input = torch.tensor(np.array(obs), dtype=torch.float32).unsqueeze(0).detach().numpy()

    agent = Agent(obs_input.shape)
    ...
    for episode in range(1, episodes + 1):
        ...
        obs_input = torch.tensor(np.array(obs), dtype=torch.float32).unsqueeze(0) / 255.0
        ...
        while True:
            ...
            next_obs_input = torch.tensor(np.array(next_obs), dtype=torch.float32).unsqueeze(0) / 255.0
