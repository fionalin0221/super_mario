...
class Agent():
    ...
    #     qvalues = self.policy_model(obs.to(device))
    ...
    def act(self, state, epsilon):
        # calcuate action values
        state = state[0].__array__() if isinstance(state, tuple) else state.__array__()
        state = torch.tensor(state, device=device)
        action_values = self.policy_model(state.to(device))  # output the values of each action
        self.qvalues.append(torch.max(action_values).item())

        # EXPLORE
        if np.random.rand() < epsilon:
            action_idx = np.random.randint(self.action_space.n)
...
def train(env, episodes):
    ...
    for episode in range(1, episodes + 1):
        ...
        obs_input = torch.tensor(np.array(obs), dtype=torch.float32).unsqueeze(0)
        ...
        while True:
            ...
            next_obs_input = torch.tensor(np.array(next_obs), dtype=torch.float32).unsqueeze(0)
